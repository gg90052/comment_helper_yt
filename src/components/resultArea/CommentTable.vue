<template>
  <table
    class="resultTable table table-auto whitespace-normal table-zebra w-full mobile:table-fixed"
  >
    <!-- head -->
    <thead>
      <tr>
        ...
        <th
          ...
          class="cursor-pointer text-center w-[6rem]"
        >
          喜歡
        </th>
        <th
          @click="sort('reply_count')"
          class="cursor-pointer text-center w-[6rem]"
        >
          回覆
        </th>
        <th
          @click="sort('time')"
          class="!rounded-t-none cursor-pointer w-[14rem]"
        >
          留言時間
        </th>
      </tr>
      ...
      <template v-for="(tr, index) in sortTableData" :key="tr.id">
        <tr>
          ...
          <td class="whitespace-normal break-all">
            <a :href="tr.author_url" class="text-[#D68927]" target="_blank">{{
              tr.name
            }}</a>
          </td>
