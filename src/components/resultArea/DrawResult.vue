<template>
  <div class="bg-white p-4 grid grid-cols-4 gap-2 mobile:grid-cols-1">
    <template v-for="(card, index) in dataStore.drawResult" :key="card.id">
      <div
        v-if="showPrizeTitle(index) && dataStore.prizeList.length > 0"
        class="col-span-4 text-white bg-blue-500 block py-2"
      >
        {{ showPrizeTitle(index).title }}：<span
          class="text-yellow-300 font-bold mx-2"
          >{{ showPrizeTitle(index).num }}</span
        >名
      </div>
      <div class="card w-full bg-base-100 border shadow-xl text-left">
        <div class="card-body relative">
          ...
          <p>
            <a
              class="text-[#D68927] hover:underline"
              :href="messageURL(card.comment_id)"
              target="_blank"
              v-html="card.comment"
            ></a>
          </p>
          <p class="absolute text-xs right-4 bottom-1">
            {{ dayjs(card.time).format("YYYY-MM-DD HH:mm:ss") }}
          </p>
        </div>
      </div>
    </template>
